...
namespace Arkanoid_02
{
    ...
    public class Player : SpriteArk
    {
        ...
        public Player(ContentManager content, string texture, Vector2 pos) : base(content, texture, pos)
        {
        ...
        public void Movement(GameTime gameTime)
        {
            ...
            if (canmove && KeyState.IsKeyDown(Keys.Left) && position.X > 35f)
            {
            ...
            if (canmove && KeyState.IsKeyDown(Keys.Right) && position.X + Size.X < 810f)
            {
                position.X += movement.X * speed *(float)gameTime.ElapsedGameTime.TotalSeconds;
                Rightmove = true;
                Leftmove = false;
